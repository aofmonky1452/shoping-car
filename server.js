...
var endpoint = 'http://localhost:3000/uploads/';
...
app.get('/api/getproduct', (req,res) => {
    try {
        db.query(`select * from sp_product`,(err,resp) => {
            db.query(`SELECT type FROM sp_product GROUP by type`, (err3, res3) => {
                // console.log(resp)
...
app.post('/api/register', (req,res) => {
    ...
    try {
        db.query(`insert into sp_user (phoneno,password,name) values (?,?,?) `,
...
app.post('/api/login', (req,res) => {
    ...
    try {
        db.query(`select * from sp_user where phoneno = ? and password = ? `,
        [phoneno,password],
        (err,resp) => {
            if(resp && resp[0]) {
                return res.status(200).json({
...
app.post('/api/deleteproduct', (req,res) => {
    ...
    try {
        db.query(`delete from sp_product where id = ? `,
        [id],
        (err,resp) => {
            if(!err) {
                return res.status(200).json({
...
app.post('/api/addproduct', (req,res) => {
    ...
    try {
        db.query(`insert into sp_product (name,img,price,description,type) values (?,?,?,?,?) `,
        [name,img,parseInt(price),detail,type],
        (err,resp) => {
            if(!err) {
                return res.status(200).json({
                    RespCode: 200,
    ...
    try {
        upload(req, res, (err2) => { 
            if(err2) {
                ...
                Jimp.read('./uploads/'+req.file.filename,  (err, res2) => {
                    if(err) throw err;
                    else {
                        res2.quality(0.1).write('./uploads/'+req.file.filename)
                        return res.status(200).json({
                            RespCode: 200,
                            RespMessage: 'good',
                            Result: endpoint + req.file.filename 
                        })
                    }
                })
            }
